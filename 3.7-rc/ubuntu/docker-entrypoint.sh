...
if [[ "$1" == rabbitmq* ]] && [ "$(id -u)" = '0' ]; then
	...
	# https://github.com/nholuongut/rabbitmq/issues/283
...
if [ "$1" = 'rabbitmq-server' ] && [ "$shouldWriteConfig" ]; then
	...
	if [ "${RABBITMQ_VM_MEMORY_HIGH_WATERMARK:-}" ]; then
		# https://github.com/nholuongut/rabbitmq/pull/105#issuecomment-242165822
	...
	if [ "$(rabbitmq-plugins list -q -m -e rabbitmq_management)" ]; then
		...
		if [ -f "$managementDefinitionsFile" ]; then
			# see also https://github.com/nholuongut/rabbitmq/pull/112#issuecomment-271485550
...
if [ "$haveSslConfig" ] && [[ "$1" == rabbitmq* ]] && [ ! -f "$combinedSsl" ]; then
	# Create combined cert
	{
		...
		echo # https://github.com/nholuongut/rabbitmq/issues/357#issuecomment-517755647
